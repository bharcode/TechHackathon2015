...
.controller('ChatDetailCtrl', function($scope, $stateParams,Friends, Merchants, $http) {
...
function onSuccess(response) {
    ...
    shippingMethods: [
        ...
        { identifier: "Airmail", detail: "Ship it by airplane.", amount: 0.01 }
            ...
            var selectId = document.getElementById('amountSelected');
            $scope.transId= LeftPadWithZeros();
            $scope.transAmt=500;
            $scope.showSelectedAmount = function(amtSelect)
            {
                $scope.transAmt=amtSelect;
            }
            //$scope.transAmt = selectId.options[selectId.selectedIndex].value;
     ...
     function successCallback(){
    	...
    	touchid.authenticate(successCallback, failureCallback, "Pay With POSify");    	
     
      var smsUrl = "http://login.smsgatewayhub.com/smsapi/pushsms.aspx?user=easysmartfood&pwd=965485&to="+"9686363762&sid=WEBSMS"+"&msg=Transaction with ID:"+$scope.transId+" Amount INR."+$scope.transAmt+" has been approved for CardNumber: 3744XXXXXX1003"+"&fl=0";
    ...
    // alert(smsUrl); 
      $http.get(smsUrl).success(function(data) {
            ...
            $scope.transIdShow = true;
             $scope.requestPayEabled = false;
            //alert(Friends.get(1)+" -- ");
            Friends.put($scope.merchant,$scope.transAmt,$scope.transAmt,$scope.transId);         
       //     alert(data);
                     })
       .error(function(data) {
    alert("ERROR");
...
.controller('scannedMerchantCtrl', function($scope, Merchants,Friends,$http) {
  $scope.merchants = Merchants.all();
  $scope.merchant = Merchants.get(4);
...
.controller('AccountCtrl', function($scope) {
 ...
 $scope.pwpChange = function() {
    	function successCallback(){
            alert("Pay With POSify registration Success");
        }
        function failureCallback(){
            alert("Success TouchID");
        }
